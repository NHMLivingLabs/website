...
DATA_FAM_FILE_CANDIDATES = [
    ROOT / "data" / "common-names.json",
    ROOT / "assets" / "data" / "common-names.json",
]
family_common_map = {}
genus_common_map = {}
...
def _page_header(
    title_text: str, subtitle: str = None, toc: bool = True, extra_lines=None
):
...
def _parse_species_include(spec_file: Path):
    ...
    try:
        ...
        if mgen:
            genus = strip_markup(
                (mgen.group(1) or mgen.group(2) or mgen.group(3) or "").strip()
            )
    except Exception:
        pass
...
for p in items:
    ...
    try:
        ...
        m = re.search(
            r"(?:tree-species-info/|_tree-species-info/_)(?:_?)([\w\-]+)\.md", text
        )
        if m:
            ...
            try:
                ...
                if old is None or inc_text != old:
                    try:
                        ...
                        print(
                            f"Warning: failed to write updated include {spec_file}: {e}"
                        )
            except (OSError, UnicodeError) as e:
...
if family_map:
    for fam_slug, fdata in sorted(
        family_map.items(), key=lambda x: x[1].get("name", "")
    ):
        fname = strip_markup(fdata.get("name"))
        fam_common = _get_family_common(fname)
        if fam_common:
        ...
        flines.append(
            f"- [{fname_display}](/trees/family/{fam_slug}.html) — {count} species"
        )
else:
...
for g_slug, gdata in genus_map.items():
    ...
    for s_slug, _ in gdata.get("species") or []:
        ...
        if f_name:
            ...
            fam_candidates.append(
                (
                    fam_display,
                    re.sub(r"[^0-9a-zA-Z-]+", "-", fam_display.lower()).strip("-"),
                )
            )
    ...
    if not fam_candidates:
        ...
        for f_slug, fdata in family_map.items():
            ...
            fam_species_names = set(
                (n or "").strip().lower() for _, n in fam_species_entries
            )
            matched = False
            for s_slug, s_name in gdata.get("species") or []:
                if (
                    s_slug in fam_species_slugs
                    or (s_name or "").strip().lower() in fam_species_names
                ):
