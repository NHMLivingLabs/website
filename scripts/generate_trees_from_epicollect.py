...
EPICOLLECT_TOKEN_URL = os.environ.get(
    "EPICOLLECT_TOKEN_URL", "https://five.epicollect.net/oauth/token"
)
...
def obtain_client_credentials_token(client_id, client_secret, token_url):
    ...
    headers = {
        ...
        "User-Agent": "nhm-urs-tree-downloader/1.0",
    ...
    if isinstance(entry, dict):
        for k, v in entry.items():
            if isinstance(k, str) and re.match(
                r"^\s*2[_\s-]*tree[_\s-]*number\s*$", k, re.IGNORECASE
            ):
...
def find_photo_filename(entry):
    ...
    def walk(o):
        if isinstance(o, dict):
            ...
            s = o.strip()
            if s and (
                s.lower().endswith((".jpg", ".jpeg", ".png"))
                or re.search(r"[0-9a-fA-F\-]{8,}_\d+\.jpg", s)
            ):
                return s
        return None
        ...
        if common:
            ...
            if inc_dir.exists():
                for f in inc_dir.iterdir():
                    ...
                    if (
                        stem == slug_common
                        or stem.endswith("-" + slug_common)
                        or stem.startswith(slug_common + "-")
                    ):
                        inc_file = f
                        break
            ...
            if inc_file and inc_file.exists():
                try:
                    ...
                    print(
                        f"Warning: failed to compute relative path for include {inc_file}: {e}"
                    )
    except Exception as e:
        # unexpected failure while searching for species include; warn and continue
        print(f"Warning: species include lookup failed for {species_raw!r}: {e}")
    if photo_path and photo_path.exists():
        img_rel = f"/assets/images/trees/{photo_path.name}"
        lines.append(f"![Tree {tree_id} - {display_name}]({img_rel})")
    else:
        lines.append(
            f"![Tree {tree_id} - {display_name}](/assets/images/trees/tree{tree_id}.jpg)"
        )
...
def download_image(project_slug, photo_filename, target_path, token=None):
    ...
    media_url = (
        f"https://five.epicollect.net/api/export/media/{project_slug}?type=photo&format=entry_original&name="
        + urllib.parse.quote(photo_filename)
    )
    headers = {
        "User-Agent": "nhm-urs-tree-downloader/1.0",
        "Accept": "image/*,*/*;q=0.8",
...
def main():
    ...
    if not token and EPICOLLECT_CLIENT_ID and EPICOLLECT_CLIENT_SECRET:
        token = obtain_client_credentials_token(
            EPICOLLECT_CLIENT_ID, EPICOLLECT_CLIENT_SECRET, EPICOLLECT_TOKEN_URL
        )
